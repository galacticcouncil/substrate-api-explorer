...
const ExploreApiBreadcrumbs = ({ match }: Props) => {
  ...
  useEffect(() => {
    ...
    const stepsFormatted = stepsArray.map((item, idx) => ({
      name: item,
      href: `/explore-api/${stepsArray.slice(0, idx + 1).join('/')}`
    }))
  ...
  return steps ? (
    <S.Wrapper>
      <S.Breadcrumb to="/explore-api">api</S.Breadcrumb>
      {steps.map((item, idx) => (
...
export default ExploreApiBreadcrumbs
